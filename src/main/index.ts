...
async function runIpcTest(): Promise<void> {
  ...
  const done = new Promise<boolean>((resolve) => {
    const timer = setTimeout(() => { console.error('[ipc-test] ❌ 超时'); resolve(false) }, 25000)
    win.webContents.on('console-message', (...args: unknown[]) => {
      ...
      if (msg.includes('ALL-DONE')) { clearTimeout(timer); resolve(true) }
      if (msg.includes('FAIL')) { clearTimeout(timer); resolve(false) }
    })
  })
...
async function runSmoke(): Promise<void> {
  ...
  const empId = await createEmployee({ emp_no: empNo, name: '冒烟测试员', dept_id: leaf.id })
  const assetId = await createAsset({
    ...
    condition_score: 8,
    components: [
      { comp_type: 'monitor', brand_model: 'Dell U2720D', spec: '27寸 4K', quantity: 2, source: 'manual', remark: '' },
      { comp_type: 'kbmouse', brand_model: 'Logitech MK470', spec: '', quantity: 1, source: 'manual', remark: '' }
    ]
  })
  ...
  const { assets, components } = await listAssetsWithComponents({ page: 1, pageSize: 100 })
  const file = await exportLedger(
    assets, components,
    path.join(app.getPath('userData'), `资产台账-smoke-${Date.now()}.xlsx`)
  )
