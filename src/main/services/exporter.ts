...
import type {
  Asset, AssetComponent, CollectRecord, DiskInfo, NicInfo
} from '../../shared/types'
...
export async function exportCollectRecords(records: CollectRecord[], filePath: string): Promise<string> {
  ...
  ws.columns = [
    ...
    { header: '主机名', key: 'hostname', width: 20 },
    { header: 'MAC 地址', key: 'mac', width: 18 },
    { header: '操作系统', key: 'os', width: 26 },
    { header: 'CPU', key: 'cpu', width: 40 },
    { header: '核数', key: 'cores', width: 8 },
    { header: '内存(MB)', key: 'mem', width: 12 },
    { header: '磁盘', key: 'disks', width: 32 },
    { header: '网卡/IP/MAC', key: 'nics', width: 46 },
    { header: '采集时间', key: 'at', width: 22 }
  ]
  ...
  for (const a of records) {
    ...
    ws.addRow({
      id: a.id, hostname: a.hostname, mac: a.mac ?? '', os: a.os,
      cpu: a.cpu, cores: a.cpu_cores, mem: a.mem_total_mb,
      disks, nics, at: a.collected_at
    })
  }
...
export async function exportLedger(
  ...
  ws.columns = [
    { header: '资产编号', key: 'no', width: 18 },
    { header: '分类', key: 'cat', width: 10 },
    { header: '归属员工', key: 'emp', width: 12 },
    { header: '部门', key: 'dept', width: 20 },
    { header: '状态', key: 'status', width: 10 },
    { header: '存放位置', key: 'loc', width: 14 },
    { header: '原值(元)', key: 'ov', width: 12 },
    { header: '成色', key: 'condition', width: 8 },
    { header: '当前净值(元)', key: 'nv', width: 14 },
    { header: '更新时间', key: 'ut', width: 20 }
  ]
  ws.getRow(1).font = { bold: true }
  ...
  ws2.columns = [
    ...
    { header: '品牌型号', key: 'model', width: 30 },
    { header: '序列号SN', key: 'sn', width: 18 },
    { header: '规格参数', key: 'spec', width: 30 },
    { header: '数量', key: 'qty', width: 8 },
    { header: '来源', key: 'src', width: 8 },
