...
function withNetValue(a: Asset): Asset {
  return { ...a, net_value: calcNetValue(a.original_value, a.condition_score) }
}
...
async function validateUniqueMac(mac: string | null | undefined, excludeId?: number): Promise<void> {
  const normalized = mac?.replace(/[:-]/g, '').replace(/\s/g, '').toLowerCase()
  ...
  for (const c of components) {
    await conn.query(
      `INSERT INTO asset_components (asset_id, comp_type, brand_model, sn, spec, quantity, source, remark)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        assetId, c.comp_type, c.brand_model ?? '', c.sn ?? '', c.spec ?? '',
        c.quantity ?? 1, c.source ?? 'manual', c.remark ?? ''
      ]
    )
  }
}
  ...
  try {
    ...
    const [r] = await conn.query<ResultSetHeader>(
      `INSERT INTO assets
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        assetNo, input.category, input.brand_model ?? '', input.sn ?? null,
        input.employee_id ?? null, input.status, input.location ?? null,
        input.hostname ?? null, input.mac ?? null, input.config_cpu ?? null, input.config_memory ?? null, input.config_disk ?? null,
        input.original_value ?? null, input.condition_score ?? 10, input.remark ?? null
      ]
    )
  ...
  try {
    ...
    await conn.query(
      `UPDATE assets SET
       ...
       WHERE id = ?`,
      [
        input.asset_no ?? '', input.category, input.brand_model ?? '', input.sn ?? null,
        input.employee_id ?? null, input.status, input.location ?? null,
        input.hostname ?? null, input.mac ?? null, input.config_cpu ?? null, input.config_memory ?? null, input.config_disk ?? null,
        input.original_value ?? null, input.condition_score ?? 10, input.remark ?? null, id
      ]
    )
...
export async function listAssets(query: AssetQuery): Promise<PagedResult<Asset>> {
  ...
  if (query.category) { where.push('a.category = ?'); params.push(query.category) }
  ...
  return { rows: await attachNames(rows as AssetRow[]), total }
}

/** 导出用：按筛选取全量资产（不分页）+ 各自的组件清单 */
export async function listAssetsWithComponents(
  query: AssetQuery
): Promise<{ assets: Asset[]; components: AssetComponent[] }> {
  const { rows } = await listAssets({ ...query, page: 1, pageSize: 10000 })
  if (rows.length === 0) return { assets: [], components: [] }
  ...
  return { assets: rows, components: comps as AssetComponent[] }
}

export async function getAsset(id: number): Promise<{ asset: Asset; components: AssetComponent[] }> {
  const pool = getPool()
  const [rows] = await pool.query<RowDataPacket[]>(
    `SELECT a.*, e.name AS emp_name, e.dept_id AS emp_dept_id
...
export async function assetStats(): Promise<AssetStats> {
  ...
  const byStatus = new Map((rows as { status: string; n: number }[]).map((r) => [r.status, r.n]))
  ...
  let totalNetValue = 0
  for (const v of values as { original_value: number; condition_score: number }[]) {
