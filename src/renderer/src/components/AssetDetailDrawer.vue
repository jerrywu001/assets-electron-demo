...
void api.listCategories().then((items) => { categories.value = items }).catch(() => undefined)
watch(
  () => props.assetId,
  async (id) => {
    ...
    <div v-loading="loading">
      <template v-if="asset">
        <el-descriptions :column="2" border size="small">
          ...
          <el-descriptions-item label="分类">{{ CATEGORY_LABELS[asset.category] }}</el-descriptions-item>
          <el-descriptions-item label="状态">
            <el-tag size="small">{{ STATUS_LABELS[asset.status] }}</el-tag>
          </el-descriptions-item>
          <el-descriptions-item label="归属员工">{{ asset.emp_name || '-' }}</el-descriptions-item>
          <el-descriptions-item label="部门">{{ asset.dept_path || '-' }}</el-descriptions-item>
          ...
          <el-descriptions-item label="原值">{{ fmtMoney(asset.original_value) }}</el-descriptions-item>
          <el-descriptions-item label="成色">{{ asset.condition_score ?? '-' }}</el-descriptions-item>
        ...
        <template v-if="hasConfig">
          <h4 style="margin: 20px 0 8px">配置信息</h4>
          <el-descriptions :column="2" border size="small">
            <el-descriptions-item label="设备名称" :span="2">{{ asset.hostname || '-' }}</el-descriptions-item>
            <el-descriptions-item label="MAC 地址">{{ asset.mac || '-' }}</el-descriptions-item>
            <el-descriptions-item label="CPU">{{ asset.config_cpu || '-' }}</el-descriptions-item>
            <el-descriptions-item label="内存">{{ asset.config_memory || '-' }}</el-descriptions-item>
            <el-descriptions-item label="硬盘">{{ asset.config_disk || '-' }}</el-descriptions-item>
          </el-descriptions>
        </template>

        <h4 style="margin: 20px 0 8px">设备清单（{{ components.length }} 行）</h4>
