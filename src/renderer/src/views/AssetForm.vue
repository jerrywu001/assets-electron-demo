...
import {
  calcNetValue, DEFAULT_CONDITION_SCORE, STATUS_LABELS
} from '../../../shared/depreciation'
import type {
  AssetCategory, AssetComponent, AssetInput, AssetStatus, CompType, Employee
} from '../../../shared/types'
...
const config = reactive({ cpu: '', memory: '', disk: '' })
...
const compTypes = computed<[CompType, string][]>(() => {
  ...
  return (category?.device_types ?? []).map((x: { value: CompType; name: string }) => [x.value, x.name])
})
const hasDeviceTypes = computed(() => compTypes.value.length > 0)
function openCategoryConfig(): void {
  const category = categories.value.find((x) => x.value === form.category)
  if (category) router.push({ path: '/categories', query: { categoryId: String(category.id) } })
}
...
async function copyConfigSummary(): Promise<void> {
  ...
  try { await navigator.clipboard.writeText(summary); ElMessage.success('摘要信息已复制') } catch (e) { ElMessage.error(errMsg(e)) }
}

function addComponent(): void {
  form.components.push({ comp_type: compTypes.value[0]?.[0] ?? 'host', brand_model: '', sn: '', spec: '', quantity: 1, source: 'manual', remark: '' })
}
...
onMounted(async () => {
  ...
  const activeEmployees = await api.listEmployees({ status: 'active', page: 1, pageSize: 200 })
  ...
  if (editId) {
    ...
    if (form.components.length === 0 && (asset.brand_model || asset.sn)) {
      form.components.push({
        comp_type: compTypes.value[0]?.[0] ?? 'host', brand_model: asset.brand_model ?? '', sn: asset.sn ?? '',
        spec: '', quantity: 1, source: 'manual', remark: ''
      })
...
<template>
  <h2 class="page-title">{{ editId ? `编辑资产 ${form.asset_no}` : '资产登记' }}</h2>
  <p class="page-sub">基本信息 / 价值信息 / 设备清单 / 备注，净值按原值和成色实时计算</p>
  ...
  <el-form class="asset-form" label-width="100px" style="width: 100%; max-width: none">
    ...
    <el-card v-if="isOfficeComputer" shadow="never" style="margin-bottom: 16px">
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center">
          <span>配置信息</span>
          <div class="config-actions"><el-tooltip v-if="configLoaded" content="复制后可填入设备清单" placement="top"><el-button size="small" @click="copyConfigSummary">复制摘要信息</el-button></el-tooltip><el-button size="small" type="success" :loading="extracting" @click="extractLocal">自动获取本机配置</el-button></div>
        </div>
      </template>
      <el-row :gutter="16">
        <el-col :span="12"><el-form-item label="设备名称"><el-input v-model="form.hostname" /></el-form-item></el-col>
      </el-row>
      <el-row :gutter="16">
        <el-col :span="12"><el-form-item label="MAC 地址"><el-input v-model="form.mac" /></el-form-item></el-col>
        <el-col :span="12"><el-form-item label="CPU"><el-input v-model="config.cpu" readonly /></el-form-item></el-col>
      </el-row>
      <el-row :gutter="16">
        <el-col :span="12"><el-form-item label="内存"><el-input v-model="config.memory" readonly /></el-form-item></el-col>
        <el-col :span="12"><el-form-item label="硬盘"><el-input v-model="config.disk" readonly /></el-form-item></el-col>
      </el-row>
    </el-card>

    <el-card shadow="never" style="margin-bottom: 16px">
      <template #header>价值信息</template>
      <el-row :gutter="16">
        <el-col :span="8">
    ...
    <el-card shadow="never" style="margin-bottom: 16px">
      <template #header>
        <div style="display: flex; justify-content: space-between; align-items: center">
          ...
          <div v-if="hasDeviceTypes">
            <el-button size="small" type="primary" @click="addComponent">新增行</el-button>
          </div>
        </div>
      </template>
      <div v-if="hasDeviceTypes">
        ...
        <el-table-column label="操作" width="112" align="center">
          <template #default="{ $index }">
            <el-tooltip content="上移" placement="top">
              <el-button link size="small" aria-label="上移" @click="moveComponent($index, -1)">↑</el-button>
            </el-tooltip>
            <el-tooltip content="下移" placement="top">
              <el-button link size="small" aria-label="下移" @click="moveComponent($index, 1)">↓</el-button>
            </el-tooltip>
            <el-tooltip content="删除" placement="top">
              <el-button link size="small" type="danger" aria-label="删除" @click="removeComponent($index)">×</el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
      ...
      <el-empty v-else description="当前资产分类尚未关联设备类型">
        <template #default><el-button type="primary" @click="openCategoryConfig">去资产分类管理关联</el-button></template>
      </el-empty>
    </el-card>
    ...
    <el-form-item class="asset-form-actions">
      <el-button type="primary" :loading="saving" @click="submit">{{ editId ? '保存' : '登记' }}</el-button>
      <el-button @click="router.push('/assets')">返回台账</el-button>
    </el-form-item>
  </el-form>
</template>
