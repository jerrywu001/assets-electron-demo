...
<template>
  ...
  <div v-if="stats" class="mb-5 grid grid-cols-2 gap-4 md:grid-cols-5">
    <div class="rounded-xl border border-app-border p-4 transition-shadow hover:shadow-sm">
      <div class="text-xs text-app-text2">
        资产总数
      </div>
    ...
    <div class="rounded-xl border border-app-border p-4 transition-shadow hover:shadow-sm">
      ...
      <div class="mt-1 text-2xl font-semibold tracking-tight text-emerald-600 dark:text-emerald-400">
        {{ stats.inuse }}
      </div>
    </div>
    <div class="rounded-xl border border-app-border p-4 transition-shadow hover:shadow-sm">
      ...
      <div class="mt-1 text-2xl font-semibold tracking-tight text-sky-600 dark:text-sky-400">
        {{ stats.idle }}
      </div>
    </div>
    <div class="rounded-xl border border-app-border p-4 transition-shadow hover:shadow-sm">
      ...
      <div class="mt-1 text-2xl font-semibold tracking-tight text-amber-600 dark:text-amber-400">
        {{ stats.repair }}
      </div>
    </div>
    <div class="rounded-xl border border-app-border p-4 transition-shadow hover:shadow-sm">
  ...
  <div class="filter-bar">
    ...
    <div class="asset-actions">
      <el-button type="success" @click="router.push('/assets/new')">
        登记资产
      </el-button>
  ...
  <div class="asset-table-wrap">
    <el-table v-loading="loading" height="100%" :data="rows" border stripe empty-text="暂无资产，点击「登记资产」">
      ...
      <el-table-column label="原值" width="110" align="right">
        <template #default="{ row }">
          {{ fmtMoney(row.original_value) }}
        </template>
      </el-table-column>
      <el-table-column prop="condition_score" label="成色" width="70" align="center" />
      <el-table-column label="当前净值" width="110" align="right">
        <template #default="{ row }">
          <b>{{ fmtMoney(row.net_value) }}</b>
        </template>
      </el-table-column>
      <el-table-column label="最近更新" width="180">
        <template #default="{ row }">
          <span class="no-wrap">{{ row.updated_at }}</span>
        </template>
      </el-table-column>
      <el-table-column label="操作" width="220" fixed="right">
        <template #default="{ row }">
          <el-button class="asset-action-link" link type="primary" @click="detailId = row.id">
            详情
          </el-button>
          <el-button v-if="row.status !== 'scrapped'" class="asset-action-link" link type="primary" @click="router.push(`/assets/${row.id}`)">
            编辑
          </el-button>
          <el-button v-if="row.status === 'pending_recycle'" link type="warning" @click="recycle(row)">
            回收
          </el-button>
          <el-button v-if="row.status !== 'scrapped'" link type="danger" @click="scrap(row)">
            报废
          </el-button>
        </template>
      </el-table-column>
