...
const employeeTreeProps = {
  label: 'name',
  children: 'children',
  disabled: (data: DepartmentNode) => data.parent_id == null || data.children?.length > 0,
};
...
<template>
  ...
  <p class="page-sub">
    员工档案与在职状态，离职自动触发资产回收流程
  </p>

  <div class="filter-bar">
    <el-input
      v-model="query.keyword" placeholder="工号 / 姓名 / 职位" clearable style="width: 200px;"
    ...
    <el-button class="create-employee" type="success" @click="openCreate">
      新增员工
    </el-button>
  </div>

  <div class="data-table-wrap employee-table-wrap">
    <el-table v-loading="loading" height="100%" :data="rows" border stripe empty-text="暂无员工">
      ...
      <el-table-column label="名下资产" width="120">
        <template #default="{ row }">
          {{ row.asset_count ?? 0 }}资产共{{ row.device_count ?? 0 }}设备
        </template>
      </el-table-column>
      <el-table-column label="操作" width="150" fixed="right">
        <template #default="{ row }">
          <el-button class="employee-edit-link" link type="primary" @click="openEdit(row)">
            编辑
          </el-button>
          <el-button v-if="row.status === 'active'" link type="danger" @click="markLeft(row)">
            标记离职
          </el-button>
        </template>
      </el-table-column>
  ...
  <el-pagination
    ...
    v-model:page-size="query.pageSize" style="margin-top: 16px; justify-content: flex-end;"
    background
...
.employee-table-wrap {
  height: clamp(300px, calc(100vh - 270px), 800px);
}
