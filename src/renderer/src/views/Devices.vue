...
onMounted(async () => {
  categories.value = await api.listCategories(); await refresh(); 
});
</script>
...
<template>
  ...
  <p class="page-sub">
    跨资产的设备平铺视图（只读）：筛「显示器」看全公司分布，筛「自动」看哪些机器采集过配置；点击资产编号跳转详情
  </p>
  ...
  <div class="filter-bar">
    ...
    <el-input
      v-model="query.keyword" placeholder="型号 / 规格 / 资产编号" clearable style="width: 200px;"
  ...
  <div class="data-table-wrap device-table-wrap">
    <el-table v-loading="loading" height="100%" :data="rows" border stripe empty-text="暂无组件数据">
      <el-table-column label="所属资产" width="210">
        <template #default="{ row }">
          <el-button class="device-asset-link" link type="primary" @click="detailId = row.asset_id ?? null">
            {{ row.asset_no }}
          </el-button>
        </template>
      </el-table-column>
      <el-table-column label="资产状态" width="90">
        <template #default="{ row }">
          <el-tag size="small">
            {{ STATUS_LABELS[row.asset_status as keyof typeof STATUS_LABELS] }}
          </el-tag>
        </template>
      </el-table-column>
