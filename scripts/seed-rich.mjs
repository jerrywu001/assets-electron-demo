...
const models = [
  ['PC', '联想 ThinkCentre M760', 6800],
  ['PC', 'Dell OptiPlex 7010', 7200],
  ['PC', 'HP ProDesk 480 G9', 6100],
  ['SRV', 'Dell PowerEdge R750', 46000],
  ['SRV', 'HPE ProLiant DL380', 52000],
  ['NET', '华为 AR6120 路由器', 8900],
  ['NET', 'H3C S5130S 交换机', 12600],
  ['PRINT', '惠普 LaserJet M607', 7600],
  ['PRINT', '佳能 imageRUNNER C3226', 14800],
  ['MOB', 'iPhone 15 Pro', 8999],
  ['MOB', '华为 MatePad Pro', 4999],
  ['AV', '爱普生 EB-992F 投影仪', 6800],
  ['FURN', '办公椅 人体工学款', 1800],
  ['OTH', '会议室白板', 1200],
];
  ...
  for (let i = count; i < 30; i += 1) {
    ...
    const [result] = await pool.query(
      ...
      [
        ...
        category,
        model,
        `DEMO-SN-${String(number).padStart(5, '0')}`,
        employeeId,
        status,
        `${1 + i % 6}F-${String.fromCharCode(65 + i % 4)}${String(10 + i).padStart(2, '0')}`,
        category === 'PC' ? `DEMO-PC-${String(i + 1).padStart(2, '0')}` : null,
        mac,
        ...config,
        value,
        5 + i % 6,
